function [N, height] = LambertianSFP(rho, phi, mask, n, s, albedo, Iun)
    ...
    theta = acos(min(real(((2 .* rho + 2 .* n .^ 2 .* rho - 2 .* n .^ 2 + n .^ 4 + rho .^ 2 + 4 .* n .^ 2 * rho .^ 2 - n .^ 4 .* rho .^ 2 - 4 .* n .^ 3 .* rho .* (- (rho - 1) .* (rho + 1)) .^ (1/2) + 1) ./ (n .^ 4 .* rho .^ 2 + 2 .* n .^ 4 .* rho + n .^ 4 + 6 .* n .^ 2 .* rho .^ 2 + 4 .* n .^ 2 .* rho - 2 .* n .^ 2 + rho .^ 2 + 2 .* rho + 1)) .^ (1/2)), 1));

    Iun = Iun ./ albedo;
    Iun(Iun > 1) = albedo(Iun > 1);
    ...
    for row = 1:size(mask, 1)
        ...
        for col = 1:size(mask, 2)
            ...
            if mask(row, col)
                ...
                c = pre_c * cos(theta(row, col)) - Iun(row, col);
                d = -sin(theta(row, col)) ^ 2;
                ny1 =- (b * c + a * (- d * a ^ 2 - d * b ^ 2 - c ^ 2) ^ (0.5)) / norm_ab;
                ny2 =- (b * c - a * (- d * a ^ 2 - d * b ^ 2 - c ^ 2) ^ (0.5)) / norm_ab;
                ...
                n1best = max(dot(n1, squeeze(N1(row, col, :))), dot(n1, squeeze(N2(row, col, :))));
                n2best = max(dot(n2, squeeze(N1(row, col, :))), dot(n2, squeeze(N2(row, col, :))));
